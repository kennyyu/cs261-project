...
Many existing intrusion detection systems have made use of the fact that intrusion detection is an example of a
{\em novelty detection} problem where access to novel or abnormal patterns is difficult to obtain. Somayaji and Forrest
attempt to solve this problem by analyzing sequences of system calls (CITE WORK). By analyzing the sequences
...
without knowing explicitly what is considered ``abnormal." 
...
&>& \psi
\end{eqnarray*}
for some threshold $0 \leq \psi \leq 1$, called the {\em false detection rate}. 
...
centrality metrics as our features. For the common variance of our Gaussian kernels, we chose
