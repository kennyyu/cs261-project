...
>>> TODO INSERT PROVENANCE GRAPH SMALL EXAMPLE HERE SHOWING DATA FLOW
...
of normal patterns \cite{parzen}. However, ???authors note that in the case of intrusion detection, novel
...
of intrusion detection, and to determine the scope of an intrusion. ??? developed Backtracker \cite{backtracker}
 a modified
...
for the offending node in the provenance graph. ??? note the following 3 observations unique to provenance graphs \cite{clustering}:
\begin{enumerate}
...
\bibitem{provstat}
\textsc{Allen, D. M., Chapman, A., Seligman, L., and Blaustein, B.} In {\em Proceedings of the International Conference on Collaborative Computing: Networking, Applications and Worksharing} (October 2011).
